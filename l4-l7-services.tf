...
_______________________________________________________________________________________________________________________
*/
resource "aci_concrete_device" "map" {
  depends_on      = [aci_l4_l7_device.map]
  for_each        = local.concrete_devices
  l4_l7_device_dn = aci_l4_l7_device.map[each.value.l4-l7-device].id
  name            = each.value.name
...
_______________________________________________________________________________________________________________________
*/
resource "aci_concrete_interface" "map" {
  depends_on         = [aci_concrete_device.map]
  for_each           = local.concrete_interfaces
  concrete_device_dn = aci_concrete_device.map[each.value.concrete_device].id
...
_______________________________________________________________________________________________________________________
*/
resource "aci_l4_l7_logical_interface" "map" {
  depends_on               = [aci_concrete_interface.map]
  ...
  relation_vns_rs_c_if_att_n = [
    for s in each.value.concrete_interfaces : aci_concrete_interface.map[s].id
  ]
}
...
_______________________________________________________________________________________________________________________
*/
resource "aci_l4_l7_service_graph_template" "map" {
  depends_on       = [aci_tenant.map]
...
resource "aci_connection" "l4_l7_service_graph_connections" {
  depends_on = [
    aci_l4_l7_service_graph_template.map
  ]
  ...
  l4_l7_service_graph_template_dn = aci_l4_l7_service_graph_template.map[each.value.l4_l7_service_graph_template].id
...
_______________________________________________________________________________________________________________________
*/
resource "aci_function_node" "map" {
  depends_on         = [aci_l4_l7_service_graph_template.map]
